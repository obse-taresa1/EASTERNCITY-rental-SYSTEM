...
const ALLOWED_NEIGHBOURHOODS = [
  ...
  "Arategna", "Jagol", "Shenkor", "Kazanchis", "Werwari", "Bate", "Botanic"
];
// Community is intentionally limited to rental-marketplace posts.  Keep the
// persisted type broad for compatibility with existing records, while categories
// define the only public Community choices.
const POST_TYPES = new Set(['COMMUNITY_FEED']);
const POST_CATEGORIES = new Set(['LOOKING_FOR_ITEM', 'EMERGENCY_REQUEST']);
...
exports.createCommunityPost = async (req, res, next) => {
  try {
    const { type, title, description, category, city } = req.body;
    if (!POST_TYPES.has(type) || !POST_CATEGORIES.has(category)) return res.status(400).json({ message: 'Choose a valid Community category.' });
    ...
    const post = await prisma.communityPost.create({
      data: {
        ...
        tags: normalizeTags(req.body.tags), authorId: req.user.id,
...
exports.getCommunityPostById = async (req, res, next) => {
  try {
    ...
    const post = await prisma.communityPost.findUnique({
      where: { id },
      include: { ...feedInclude(req.user?.id), comments: { include: { author: { select: authorSelect } }, orderBy: { createdAt: 'desc' }, take: 50 } },
    });
    const isAdmin = req.user && ['ADMIN', 'SUPER_ADMIN'].includes(req.user.role);
    const isPubliclyViewable = ['APPROVED', 'RESOLVED'].includes(post.status);
    if (!post || (!isAdmin && !isPubliclyViewable && post.authorId !== req.user?.id)) return res.status(404).json({ message: 'Community request not found.' });
