...
module.exports = {
  ...
  getSettings: handler((req) => service.getSettingsForRole(req.user.role)),
  listBookings: handler((req) => service.listBookings(req.query)),
  listCategories: handler(() => service.listCategories()),
  listCommunityPosts: handler((req) => service.listCommunityPosts(req.query)),
  deleteCommunityPost: handler((req) => service.deleteCommunityPost(req.user, req.params.id)),
  ...
  savePlatformSettings: handler((req) => service.savePlatformSettings(req.user, req.body)),
  saveAdminPreferences: handler((req) => service.saveAdminPreferences(req.user, req.body)),
  updateCommunityPost: handler((req) => service.updateCommunityPost(req.user, req.params.id, req.body)),
