    ...
    <main className="dashboard-content">
              ...
              {[
                ...
                <div className="col-6 mb-3" key={label}>
                  <div className="p-3 border rounded">
                    <span className="text-muted d-block mb-1"><small>{label}</small></span>
          ...
          <div className="admin-table-container">
            ...
            <ul className="list-group list-group-flush mt-2">
              <li className="list-group-item bg-transparent text-color d-flex justify-content-between">
                ...
                <strong className="text-success">{counts.totalUsers || 0}</strong>
              </li>
              <li className="list-group-item bg-transparent text-color d-flex justify-content-between">
                <span>Active Renters</span>
                <strong className="text-color">{counts.totalRenters || 0}</strong>
              </li>
              <li className="list-group-item bg-transparent text-color d-flex justify-content-between">
                <span>Active Owners</span>
                <strong className="text-color">{counts.totalOwners || 0}</strong>
              </li>
              <li className="list-group-item bg-transparent text-color d-flex justify-content-between">
                <span>Verified Requests</span>
                <strong className="text-danger">{counts.approvedVerifications || 0}</strong>
              </li>
            </ul>
          </div>
        </div>
      ...
      <div className="row">
        <div className="col-lg-6 mb-4">
          <div className="admin-table-container">
            <h2 className="h5 mb-3 d-flex align-items-center gap-2">
              <i className="bi bi-graph-up-arrow text-primary-custom" /> Platform Growth Chart
            </h2>
              ...
              <div className="d-flex align-items-end justify-content-center gap-3" style={{ height: "150px" }}>
                {chartValues.slice(-4).map((value, index) => (
                  <div
                    ...
                    style={{ height: `${Math.max(10, Math.round((Number(value) || 0) / maxChartValue * 100))}%`, width: "30px" }}
                    title={`Period ${index + 1}`}
                    key={`${value}-${index}`}
                  />
                ))}
              </div>
              <p className="mt-3 mb-0 text-muted"><small>Growth is calculated from live platform records.</small></p>
            </div>
          </div>
        </div>
        ...
        <div className="col-lg-6 mb-4">
          <div className="admin-table-container">
            <h2 className="h5 mb-3 d-flex align-items-center gap-2">
              <i className="bi bi-cash-stack text-primary-custom" /> Promotion Revenue
            </h2>
            <div className="p-4 bg-light text-dark rounded text-center">
              <strong className="h2 text-success">
                {Number(revenue.promotionRevenue || 0).toLocaleString()} ETB
              </strong>
              <p className="mt-3 mb-0 text-muted"><small>Promotion revenue comes from approved promotion records.</small></p>
            </div>
          </div>
        </div>
      </div>
