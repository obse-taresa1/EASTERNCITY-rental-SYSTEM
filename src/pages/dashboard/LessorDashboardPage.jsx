...
import { deleteOwnerListing, getAllItems, promoteOwnerListing, renewOwnerListing } from "../../services/itemService.js";
import { formatCurrency, formatDailyPrice } from "../../utils/currency.js";

const statusTabs = [
  { key: "active", label: "Active Listings", statuses: ["published", "active", "approved", "renewed"] },
...
const revenueCards = [
  { label: "Subscription Revenue", value: "18,500 ETB", icon: "bi-person-badge", tone: "success" },
  { label: "Featured Listing Revenue", value: "9,800 ETB", icon: "bi-megaphone", tone: "warning" },
  { label: "Verification Revenue", value: "3,200 ETB", icon: "bi-patch-check", tone: "success" },
  { label: "Advertisement Revenue", value: "12,400 ETB", icon: "bi-badge-ad", tone: "info" },
];
...
export default function LessorDashboardPage() {
  ...
  const totalInquiries = visibleItems.reduce((sum, item) => sum + Number(item.inquiries || 0), 0);
  const activeListings = visibleItems.filter((item) => ["published", "active", "featured", "renewed"].includes(normalizeStatus(item))).length;
  ...
  const proPlan = visibleItems.some((item) => item.subscriptionPlan === "Pro Plan");
  const verified = visibleItems.some((item) => item.verificationStatus === "verified");

  function handleDelete(item) {
    if (!item.id.startsWith("owner-listing-")) {
      setNotice("Seed listings are protected demo records. New owner listings can be deleted.");
      return;
    }
  ...
  return (
    <main className="dashboard-content owner-workspace">
      <div className="dashboard-header owner-workspace-header">
        ...
        <Link to="/list-item" className="btn btn-accent-custom btn-shine owner-add-listing">
      ...
      <div className="row g-4 my-4">
        <div className="col-md-3">
          <DashboardStatCard icon="bi-box-seam" label="Total Listings" value={visibleItems.length} />
        </div>
        <div className="col-md-3">
          <DashboardStatCard icon="bi-stars" label="Featured" value={featuredCount} tone="warning" />
        </div>
        <div className="col-md-3">
          <DashboardStatCard icon="bi-chat-dots" label="Inquiries" value={visibleItems.reduce((sum, item) => sum + (item.inquiries || 0), 0)} tone="success" />
        </div>
        <div className="col-md-3">
          <DashboardStatCard icon="bi-wallet2" label="Owner Earnings" value={formatCurrency(totalEarnings)} tone="warning" />
        </div>
      </div>
      ...
      <section className="dashboard-section owner-revenue-section">
        <div className="owner-section-heading">
          <div>
            ...
            <p className="owner-muted mb-0">Free entry stays open while upgrades, verification, promotions, and ads create revenue.</p>
          </div>
          <div className="owner-action-row">
            <button type="button" className="btn btn-outline-danger">
              <i className="bi bi-arrow-up-circle" /> Upgrade Plan
            </button>
            <button type="button" className="btn btn-accent-custom btn-shine" onClick={() => visibleItems[0] && handlePromote(visibleItems[0])}>
        ...
        <div className="owner-revenue-grid">
          ...
          <div className="owner-monetization-card">
            ...
            <small>Pro unlocks unlimited listings, credits, support, and insights.</small>
          </div>
          <div className="owner-monetization-card">
          ...
          <div className="owner-monetization-card">
            ...
            <small>Homepage and category banner placements are ready for sponsors.</small>
          </div>
        </div>

        <div className="promotion-package-row">
          {promotionPackages.map((pack) => (
            <button type="button" className="promotion-package" key={pack.label} onClick={() => visibleItems[0] && handlePromote(visibleItems[0], pack.label)}>
      ...
      <section className="dashboard-section mt-4">
        <div className="owner-section-heading">
          <div>
            ...
            <p className="owner-muted mb-0">Draft to pending approval to published, then featured, expired, and renewed.</p>
          </div>
        </div>
        <div className="status-flow">
          {["Draft", "Pending Approval", "Approved", "Published", "Featured", "Expired", "Renewed"].map((step, index) => (
      ...
      <section className="dashboard-section mt-4">
        <div className="owner-section-heading">
          <div>
            ...
            <p className="owner-muted mb-0">Manage edits, renewals, promotions, inquiries, and renter messages from one page.</p>
          </div>
        </div>

        <div className="owner-tabs" role="tablist" aria-label="Listing status filters">
          {statusTabs.map((tab) => (
            <a href={`#${tab.key}-listings`} className="owner-tab" key={tab.key}>
              {tab.label}
              <span>{visibleItems.filter((item) => tab.statuses.includes(normalizeStatus(item))).length}</span>
            </a>
          ))}
        </div>
        ...
        <div className="owner-listing-stack">
          {statusTabs.map((tab) => {
            const tabItems = visibleItems.filter((item) => tab.statuses.includes(normalizeStatus(item)));

            return (
              <div id={`${tab.key}-listings`} className="owner-listing-group" key={tab.key}>
                <h3>{tab.label}</h3>
                {tabItems.length ? (
                  <div className="owner-listing-grid">
                    {tabItems.map((item) => (
                      <article className="owner-listing-card premium-glass-card" key={item.id}>
                        <img src={item.image} alt={item.title} />
                        <div className="owner-listing-body">
                          <div className="owner-card-topline">
                            <span className={`owner-status owner-status-${normalizeStatus(item).replaceAll(" ", "-")}`}>
                              {statusLabel(normalizeStatus(item))}
                            </span>
                            {item.featured && <span className="owner-featured-badge">Featured</span>}
                          </div>
                          <h4>{item.title}</h4>
                          <p>{formatDailyPrice(item.pricePerDay)} · {item.location}</p>
                          <div className="owner-listing-meta">
                            <span><i className="bi bi-chat-dots" /> {item.inquiries || 0} inquiries</span>
                            <span><i className="bi bi-envelope" /> {item.messages || 0} messages</span>
                            <span><i className="bi bi-calendar2-week" /> Expires {item.expiresAt}</span>
                          </div>
                          <div className="owner-requirements-preview">
                            <strong>Rental requirements</strong>
                            <span>{item.requirements?.documents?.join(", ") || "Owner review required"}</span>
                          </div>
                          <div className="owner-card-actions">
                            <Link to="/list-item" className="btn btn-sm btn-outline-dark"><i className="bi bi-pencil" /> Edit</Link>
                            <button type="button" className="btn btn-sm btn-outline-danger" onClick={() => handleDelete(item)}><i className="bi bi-trash" /> Delete</button>
                            <button type="button" className="btn btn-sm btn-outline-secondary" onClick={() => handleRenew(item)}><i className="bi bi-arrow-clockwise" /> Renew</button>
                            <button type="button" className="btn btn-sm btn-accent-custom" onClick={() => handlePromote(item)}><i className="bi bi-megaphone" /> Promote</button>
                          </div>
                        </div>
                      </article>
                    ))}
                  ...
                  <div className="owner-empty-state">No {tab.label.toLowerCase()} yet.</div>
                )}
              </div>
            );
          })}
      ...
      <section className="dashboard-section mt-4">
        <div className="owner-section-heading">
          <div>
            ...
            <p className="owner-muted mb-0">Prepared analytics for subscriptions, featured listings, verification, and advertisements.</p>
          </div>
        </div>
        <div className="row g-3">
          {revenueCards.map((card) => (
            <div className="col-sm-6 col-xl-3" key={card.label}>
              <DashboardStatCard icon={card.icon} label={card.label} value={card.value} tone={card.tone} />
            </div>
          ))}
        </div>
      </section>
