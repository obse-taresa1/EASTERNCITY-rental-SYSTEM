...
export default function DashboardSidebar() {
  ...
  const activeBookings = bookings.filter((b) =>
    ["pending", "awaiting_verification", "confirmed", "active"].includes(
      b.status,
    ),
  ).length;
  const finishedRentals = bookings.filter(
    ...
    <aside className="dashboard-sidebar">
      ...
      >
        <h3 className="m-0 fw-bold text-dark">
          EASTERN<span className="text-danger">CITY</span>
        </h3>
      </NavLink>
      ...
      <div className="premium-glass-card bg-white p-3 mb-4 text-center rounded-4 shadow-sm border">
        <div className="position-relative d-inline-block mb-3 mt-2">
          {activeUser?.profileImage ? (
            <img
              ...
              style={{
                ...
                height: "80px",
                objectFit: "cover",
                border: "3px solid var(--motorx-red)",
              }}
            />
          ) : (
            <div
              className="rounded-circle shadow-sm d-flex align-items-center justify-content-center text-white fw-bold mx-auto"
              style={{
                width: "80px",
                height: "80px",
                fontSize: "2rem",
                background: "var(--motorx-red)",
              }}
            >
          ...
          <span
            className="position-absolute bottom-0 end-0 bg-success text-white rounded-circle d-flex align-items-center justify-content-center border border-white"
        ...
        <h5 className="fw-bold m-0 mb-1">
          {activeUser?.name || "Verified User"}
        </h5>
        ...
        <div className="text-start border-top pt-3 text-muted small">
          <div
            className="d-flex align-items-center gap-2 mb-2 text-truncate"
            title={activeUser?.email || "user@example.com"}
          >
          ...
          <div className="d-flex align-items-center gap-2 text-truncate">
            ...
            {activeUser?.phone || "+251 900 000 000"}
          </div>
        </div>
      </div>
      ...
      <div className="sidebar-summary-grid mb-4">
        <div className="premium-glass-card bg-white p-2 rounded-3 border text-center shadow-sm summary-stat-card">
          ...
          <span className="text-muted" style={{ fontSize: "0.7rem" }}>
            <i className="bi bi-calendar-event"></i> Active Bookings
          </span>
        </div>
        <div className="premium-glass-card bg-white p-2 rounded-3 border text-center shadow-sm summary-stat-card">
      ...
      <div className="premium-glass-card bg-white p-2 mb-4 rounded-3 border text-center shadow-sm summary-stat-card">
        <div className="d-flex justify-content-between align-items-center px-2">
          <span className="text-muted fw-bold" style={{ fontSize: "0.85rem" }}>
            <i className="bi bi-flag-fill text-success me-2"></i>Finished
            Rentals
          </span>
      ...
      <nav className="dashboard-nav">
        <NavLink to={dashboardLink} className="rounded-3 mb-1">
          <i className="bi bi-speedometer2" /> <span>Dashboard</span>
        </NavLink>
        {(role === "lessor" || role === "both") && (
          <NavLink to="/lessor-dashboard" className="rounded-3 mb-1">
            <i className="bi bi-card-checklist" /> <span>My Listings</span>
          </NavLink>
        )}
        {(role === "lessor" || role === "both") && (
          <NavLink to="/list-item" className="rounded-3 mb-1">
            <i className="bi bi-plus-circle" /> <span>Add New Listing</span>
          </NavLink>
        )}
        <NavLink to="/my-bookings" className="rounded-3 mb-1">
          <i className="bi bi-calendar-check" /> <span>My Bookings</span>
        </NavLink>
        <NavLink to="/wishlist" className="rounded-3 mb-1">
          <i className="bi bi-heart" /> <span>Wishlist / Saved Items</span>
        </NavLink>
        <NavLink to="/messages" className="rounded-3 mb-1">
          <i className="bi bi-chat-dots" /> <span>Messages</span>
        </NavLink>
        <NavLink to="/contact-owner" className="rounded-3 mb-1">
          <i className="bi bi-person-lines-fill" /> <span>Contact Owner</span>
        </NavLink>

        <div
          className="mt-4 mb-2 ps-3 text-muted"
          style={{
            fontSize: "0.7rem",
        ...
        <NavLink to="/help-center" className="rounded-3 mb-1">
          <i className="bi bi-question-circle" /> <span>Help Center</span>
        </NavLink>
      </nav>
      ...
      <div className="mt-auto pt-4">
        <button
          type="button"
          className="dashboard-logout w-100 rounded-pill fw-bold"
          onClick={logout}
        >
