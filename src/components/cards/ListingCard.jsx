...
export default function ListingCard({ item }) {
  ...
  const specs = item.specs || [
    { icon: "bi-geo-alt", label: item.sefar ? `${item.city} • ${item.sefar}` : (item.city || "EasternCity") },
    { icon: "bi-clock", label: t('perDay') },
  ];
  const categoryName = categories.find((category) => category.id === item.category)?.name || item.category;

  return (
    <article className="premium-glass-card listing-card-premium">
      <div className="card-img-wrapper">
        <img src={item.image} alt={item.imageAlt || item.title} className="card-img" />
        <div className="card-badges">
          {item.featured && <span className="badge-featured">{t('featured')}</span>}
          {item.city && (
            <span className="badge-city">
              <i className="bi bi-geo-alt-fill"></i> {item.city}{item.sefar ? ` • ${item.sefar}` : ''}
            </span>
          )}
        </div>
        {item.photos && (
      ...
      <div className="card-body-premium">
        <div className="card-header-top">
          <span className="card-category">
            {item.categoryKey ? t(item.categoryKey) : categoryName || t('rentalItem')}
          </span>
          {item.rating && (
        ...
        <div className="card-owner-info">
          ...
          <span className="owner-name">{item.ownerName || 'Verified Owner'}</span>
        ...
        <div className="card-specs-grid">
          {specs.map((spec, index) => (
            <span key={`${item.id}-${spec.labelKey || spec.label || index}`} className="spec-item">
        ...
        <div className="card-footer-premium">
          ...
          <div className="card-icon-actions">
            <button type="button" className="btn-icon-soft" aria-label={t('compare')}>
              <i className="bi bi-arrow-left-right"></i>
            </button>
            <button type="button" className="btn-icon-soft" aria-label={t('wishlist')}>
